describe Api::Controllers::Contracts::Destroy do
  ...
  describe '#call' do
    describe 'with authorization' do
      ...
      it 'destroys contract with id and user_id' do
        expect(repository).to receive(:find_by_id_and_user_id).with(1, 2).and_return(contract)
        expect(repository).to receive(:delete).with(1).and_return(contract)
      ...
      describe 'when contract was not found' do
        ...
        it 'does not delete contract' do
          expect(repository).to receive(:find_by_id_and_user_id).and_return(nil)
          expect(repository).not_to receive(:delete).with(1)
          expect { action.call(params) }.to raise_error(Api::Errors::NotFoundError)
        end
      end
    end
    ...
    describe 'without authorization' do
      ...
      it 'does not persist contract' do
        expect(repository).not_to receive(:delete)
